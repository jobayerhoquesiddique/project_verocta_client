...
def upload_file():
    ...
    try:
        ...
        if file and file.filename and allowed_file(file.filename):
            ...
            # Parse CSV file
            logging.info(f"Parsing CSV file: {filename}")
            ...
            score_label = tier_info['tier']
            score_color = tier_info['color']
            ...
            analysis_data = {
                ...
                'filename': filename,
                'green_reward_eligible': tier_info.get('green_reward_eligible', False),
...
def download_pdf():
    ...
    try:
        ...
        if os.path.exists(pdf_path):
            return send_file(pdf_path, as_attachment=True, download_name='verocta_financial_report.pdf')
        else:
            flash('PDF report not found', 'error')
            return redirect(url_for('index'))
    except Exception as e:
        logging.error(f"Error downloading PDF: {str(e)}")
        flash('Error downloading PDF report', 'error')
        return redirect(url_for('index'))
